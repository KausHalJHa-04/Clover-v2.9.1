...
const backendOutput = {
    ...
    MONGO_DATABASE: 'kaushal',
    MAILER_ENABLED: false,
};

let frontendOutput = {
    VITE_SITE_TITLE: 'kaushal',
    VITE_BACKEND_URL: 'http://localhost',
    VITE_DEMO: false,
    VITE_SITE_BRAND: 'kaushalsides',
...
(async () => {
    ...
    console.log("");
    console.log("kaushalsides".yellow);
    console.log(`kaushal v${pkg.version} Installer`.yellow);
    console.log("");
    ...
    if (arg === 'setup') {
        ...
        console.log('kaushal needs to know the public ip address of your machine.'.cyan);
        ...
        console.log('');
        console.log('kaushal requires a domain, such as elderberry.example.com.'.cyan);
    ...
    if (['setup', 'rebuild'].includes(arg)) {
        ...
        if (!os.includes('Ubuntu')) {
            ...
            console.log('If you are running this on Ubuntu, please contact kaushalsides Support.');
        ...
        if (!version.includes('22.04') && !version.includes('20.04') && !version.includes('18.04')) {
            ...
            console.log('If you are running this on a correct version, please contact kaushalsides Support.');
            return process.exit(0);
        }
        ...
        console.log(`${arg === 'setup' ? 'installing' : 'rebuilding'} kaushal backend...`.yellow);
        ...
        try {
            exec('pm2 delete --silent kaushal', {stdio : 'pipe'});
        } catch (e) {}
        exec('cd ../backend && pm2 start index.js --name kaushal');
        exec('pm2 save');
        exec('pm2 startup');
        console.log('kaushal backend started'.green);

        console.log('');

        console.log(`${arg === 'setup' ? 'installing' : 'rebuilding'} kaushal frontend...`.yellow);
        ...
        console.log('kaushal frontend ok'.green);
    }
    ...
    if (arg === 'restart') {
        try {
            exec('pm2 restart kaushal', {stdio : 'pipe'});
        } catch (e) {}
    }

    if (arg === 'stop') {
        try {
            exec('pm2 stop kaushal', {stdio : 'pipe'});
        } catch (e) {}
    }
    ...
    if (arg === 'stop') {
        console.log("");
        console.log(`kaushal has been stopped.`.green);
        console.log("");
    } else {
        console.log("");
        console.log(`kaushal v${pkg.version} ${arg === 'setup' ? 'setup' : 'restart'} complete!`.green);
        if (arg === 'setup') {
            console.log(`You should now be able to access kaushal at ${domain}`);
        }
        console.log("");
    }
})();
